   ...
   protected:
    ...
    OnListenInternal(const flutter::EncodableValue* arguments,
      ...
      parent_->dxcore_->SetProgressCallback([this, parent = parent_](const std::string& msg) {
        ...
        if (parent->ParseVPNEvent(msg, event_type)) {
          parent->HandleJSONEvent(event_type);
        }
      });
...
void VPNChannelHandler::HandleJSONEvent(const std::string& event_type) {
  if (event_type == "TUNNEL_CONNECTED") {
    ...
    PostMessage(window_handle_, WM_STATUS_RESULT, 0,
                reinterpret_cast<LPARAM>(new std::string(vpn_status_)));

    if (system_tray_) {
      system_tray_->UpdateIcon(SystemTray::TrayIconStatus::Connected);
      system_tray_->UpdateTooltip(L"DefyxVPN - Connected");
    ...
    if (system_tray_) {
      ...
      system_tray_->UpdateTooltip(L"DefyxVPN - Error");

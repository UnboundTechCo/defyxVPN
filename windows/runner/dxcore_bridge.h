...
class DXCoreBridge {
 public:
  ...
  void StartVPN(const std::string& cache_dir, const std::string& flow_line,
                const std::string& pattern, const std::string& geoipFile, const bool deepScan, const bool health_check);
  int StopVPN();
  void SetAsnName();
 ...
 private:
  ...
  using WinStartVPN_t = void (*)(const char*, const char*, const char*, const char*, int, int);

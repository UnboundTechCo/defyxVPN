...
class InternalAdStrategy implements AdLoadingStrategy {
  ...
  Future<void> _loadConnectedInternalAd(Ref ref) async {
    await Future.delayed(const Duration(milliseconds: 2500));
    debugPrint(
      '⏱️ Network routing delay complete (2.5s) - loading fresh internal ad',
    );
    ...
    try {
      ...
      debugPrint('❌ Error in delayed internal ad load flow: $e');
    }
  }
  ...
  Future<AdLoadResult> loadAd({required Ref ref}) async {
    try {
      ...
      ref
          .read(adsProvider.notifier)
          .setAdLoadFailed(errorCode: 'LOAD_ERROR', errorMessage: e.toString());
      return AdLoadResult.failure(
  ...
  Widget buildAdWidget({
    ...
    if (imageUrl.isEmpty ||
      ...
      if (imageUrl.isNotEmpty) {
        debugPrint(
          '❌ Invalid URL in buildAdWidget, refusing to render: $imageUrl',
        );
      }
            ...
            unawaited(
              Future<void>(() async {
                try {
                  ...
                  debugPrint('❌ Failed to process internal ad image error: $e');
                }
              }),
            );
          }
  ...
  void onConnectionStateChanged({
    ...
    debugPrint(
      '📍 InternalAdStrategy - Connection: ${previous.name} → ${current.name}',
    );
    ...
    if (current == ConnectionStatus.connected &&
      ...
      debugPrint(
        '▶️ Connected - will load internal ad after network routing stabilizes',
      );
      ...
      unawaited(_loadConnectedInternalAd(ref));
      return;
    }
